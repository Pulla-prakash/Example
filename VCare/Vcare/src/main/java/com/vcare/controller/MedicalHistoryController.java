...
public class MedicalHistoryController {
	...
	public String fileUpload(@RequestParam("image") MultipartFile[] file, HttpSession session, Model model,
	...
	public String view(Model model, @PathVariable("id") int id, MedicalHistory medObj) {
		log.info(id);
		Patients patient = patientsService.getPatientById(id);
		model.addAttribute("patientObj", patient.getPatientId());
		...
		log.info(medicalHistoryList.get(0).getHistoryPath());
	...
	public String deletedoctors(Model model, @PathVariable("id") int id) {
