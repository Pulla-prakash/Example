...
//			if (id == Dlist.get(i).getHospitalBranchId()) {
//
...
//			}
	...
	public String viewdoctor(Model model, @PathVariable("pid") int pid, @PathVariable(value = "doctorId") int doctorId,
			@PathVariable(value = "hbid") int hbid, Rating drating, Date timeStart, Date timeEnd,HttpServletRequest request)throws Exception {
			...
			System.out.println("formatter.parse(docAvailable.get(0).getStartTimings())===="+formatter.parse(docAvailable.get(0).getStartTimings()));
			...
			List<Appointment> app = appointmentRepository.appointmentListByDoctorid(doctorId,
	...
	public String selectBranch(Model model, Doctor objdoctor, final HttpServletRequest request,
		...
		if (getQry != null && getQry.equals("selectBranch") && serviceName != null && serviceName != "") {
			...
			model.addAttribute("noOfService", 1);
...
//		}
		...
		model.addAttribute("objectdoctor", objDoc);
	...
	public String uploaddoctorProfiles(Model model, Doctor doctorObj,@RequestParam("file") MultipartFile file ,@RequestParam("file1") MultipartFile file1,			@RequestParam("file2") MultipartFile file2 ,
			final HttpServletRequest request ) throws IOException, Exception {
	...
	public String viewUodateddoctorProfile(Model model, @PathVariable("did") int did,
		...
		model.addAttribute("noOfService", "");
		model.addAttribute("HospitalBranchMap", HospitalBranchMap);
		model.addAttribute("ServieMap", ServieMap);
		model.addAttribute("name", doctorObj.getDoctorName());
		model.addAttribute("objMailId", doctorObj.getDoctorMailId());
	...
	model.addAttribute("rating", list);
	model.addAttribute("drating", drating);
	model.addAttribute("doc", doc); double average;
	long totalrating;
	// Getting Average rating for doctor
	if (doctorRatingRepository.sumOfRating(doc.getDoctorId()) == null) {
	totalrating = 0;
	...
	List<String> times = new ArrayList<>();//To store slots
	Calendar calendar = Calendar.getInstance();//get present calendar
	calendar.setTimeZone(TimeZone.getTimeZone("Asia/Calcutta")); int minutes = calendar.get(Calendar.MINUTE);
	// to Store dates
	List<LocalDate> datesInRange = new ArrayList<>();
	//
	List<String> slots = new ArrayList<>();
	...
	LocalDate today = LocalDate.parse(request.getParameter("date")); System.out.println("today=" + today);
	System.out.println("selectedDate=" + selectedDate);
	//STORES SLOTS
	ArrayList<String> timeList = new ArrayList<>();
	if (selectedDate.equals(null)) {
	...
	//System.out.println("+++++DF++++++" + df.format(cal.getTime())); /*String t = timeStart.getHours() + ":" + timeStart.getMinutes();
	...
	if (totalSlots.contains(df.format(cal.getTime()).toString())) { System.out.println("app.contains(df.format(cal.getTime()).toString()=="
	...
	if (cal.getTime().getHours() == calObj.get(Calendar.HOUR_OF_DAY)) {
	if (cal.getTime().getMinutes() >= calObj.get(Calendar.MINUTE)) { timeList.add(df.format(cal.getTime())); model.addAttribute("times", timeList);
	}
	}
	}
	...
	timeList.add(df.format(cal.getTime()));
	cal.add(Calendar.MINUTE, 10);
	model.addAttribute("times", timeList); } else { if (totalSlots.contains(df.format(cal.getTime()).toString())) {
	System.out.println("cal.getTime()==" + cal.getTime());
	cal.add(Calendar.MINUTE, 10);
	continue;
	} else { timeList.add(df.format(cal.getTime()));
	cal.add(Calendar.MINUTE, 10);
	model.addAttribute("times", timeList);
	}
	}
	...
	/* model.addAttribute("map", map); */
	model.addAttribute("monthdate", monthdate);
	model.addAttribute("days", days); return "doctorsscreen"; }
